...
open class AbsTopPopWindow(
    ...
    open fun showPopupWindow() {
        ...
        Log.d(
            TAG,
            "showPopupWindow: ${mContentView?.isAttachedToWindow} $this isshowing: ${isShowing()}"
        )
    ...
    open fun dismiss() {
        shown = false
        windowGravity?.let { windowGravity -> runWindowAnim(windowGravity, false) }
            ?: run {
                removeViews()
    ...
    fun runWindowAnim(gravity: WindowGravity, isEnter: Boolean) {
        ...
        Log.d(
            TAG,
            "runWindowAnim() called with: startValue = $startValue, endValue = $endValue  , isEnter = $isEnter"
        )
    ...
    fun clear() {
        if (mContentView != null && windowManager != null && mContentView!!.isAttachedToWindow) {
            windowManager?.removeViewImmediate(mContentView)
        }
    }
    ...
    class Builder(
        ...
        private var provider: ViewOutlineProvider? =
        ...
        fun build(type: WindowType): AbsTopPopWindow {
            val window =
                when (type) {
                    is WindowType.Volume ->
                        TopBarVolumeWindow(context, width, height, gravity, layoutResId, typeParam)
                    is WindowType.Search ->
                        TopBarGlobalSearchWindow(
                            context,
                            width,
                            height,
                            gravity,
                    ...
                    is WindowType.Overview ->
                        AppOverviewWindow(context, width, height, gravity, layoutResId, typeParam)
                    is WindowType.SingleNotification ->
                        SingleNotificationWindow(
                            context,
                            width,
                            height,
                            gravity,
                    ...
                    is WindowType.Notification ->
                        TopBarNotificationWindow(
                            context,
                            width,
                            height,
                            gravity,
                    ...
                    is WindowType.Power ->
                        TopBarPowerWindow(context, width, height, gravity, layoutResId, typeParam)
                    is WindowType.Control ->
                        TopBarControlWindow(context, width, height, gravity, layoutResId, typeParam)
                    is WindowType.DockContext ->
                        DockContextWindow(context, width, height, gravity, layoutResId, typeParam)
                    is WindowType.About ->
                        AboutWindow(context, width, height, gravity, layoutResId, typeParam)
                    is WindowType.Default ->
