...
class TopBarControlWindow(
    ...
    companion object {
        ...
        const val SYSUI_SCREENRECORD_LAUNCHER =
    ...
    private val brightnessChangeListener =
        object : OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                Log.d(
                    TAG,
    ...
    private fun initViews() {
        ...
        if (isRecording) {
            recordTextView?.text =
                getContext().resources.getString(R.string.finish_recordscreen_string)
        } else {
        ...
        try {
            ...
            wifiImage?.apply {
                setBackgroundResource(
                    if (wifiStatus == 1) R.drawable.control_oval_blue
                    else R.drawable.control_oval_gray_22
                )
                setImageResource(
                    if (wifiStatus == 1) R.drawable.icon_wifi_select_full
                    else R.drawable.icon_wifi_select_empty
                )
            }
    ...
    override fun onClick(v: View?) {
        ...
        if (v == screenshotBtn) {
            ...
            handler.postDelayed(
                {
                    ...
                    screenshotHelper.takeScreenshot(2, 2, handler, null)
                },
                300
            )
        } else if (v == recordBtn) {
            ...
            wifiImage?.apply {
                setBackgroundResource(
                    if (wifiStatus == 0) R.drawable.control_oval_blue
                    else R.drawable.control_oval_gray_22
                )
                setImageResource(
                    if (wifiStatus == 0) R.drawable.icon_wifi_select_full
                    else R.drawable.icon_wifi_select_empty
                )
            }
        } else if (v == bleLayout) {
    ...
    private fun initBle() {
        ...
        bleImage?.apply {
            setBackgroundResource(
                if (enabled == true) R.drawable.control_oval_blue
                else R.drawable.control_oval_gray_22
            )
            setImageResource(
                if (enabled == true) R.drawable.icon_ble_select else R.drawable.icon_ble_unselect
            )
        }
            ...
            bluetoothAdapter.getProfileProxy(
                ...
                object : BluetoothProfile.ServiceListener {
                    override fun onServiceConnected(profile: Int, proxy: BluetoothProfile) {
                        ...
                        //                    tvBleName?.text =
                        // getContext().resources.getString(R.string.enabled)
                        if (profile == BluetoothProfile.A2DP) {
                            ...
                            //                        bluetoothAdapter.closeProfileProxy(profile,
                            // proxy)
                        }
                    }
                ...
                if (action == BluetoothAdapter.ACTION_STATE_CHANGED) {
                    ...
                    when (state) {
                        BluetoothAdapter.STATE_OFF -> {
                            ...
                            handler.postDelayed(Runnable { initBle() }, 500)
                        }
    ...
    fun onScreenRecordStateChange(state: Int, groupKey: String?) {
        Log.d(
            TAG,
            "onScreenRecordStateChange() called with: groupKey = $groupKey state = $state  ${groupKey?.contains("screen_record_saved")}"
        )
        if (state == NOTIF_BASE_ID || state == PROGRESS_NOTIF_ID) {
            ...
            isRecording = false
            recordTextView?.text = getContext().resources.getString(R.string.recordscreen_string)
            Toast.makeText(getContext(), R.string.success_recordscreen_string, Toast.LENGTH_SHORT)
                .show()
