...
constructor(
    ...
    public class DockContextAdapter(private val context: Context) :
            ...
            if (viewType == TYPE_APP) {
                dockContextLayout =
                    LayoutInflater.from(context)
                        .inflate(R.layout.layout_dock_context_item_app, parent, false) as ViewGroup
            } else if (viewType == TYPE_NAME) {
                dockContextLayout =
                    LayoutInflater.from(context)
                        .inflate(R.layout.layout_dock_context_item_action, parent, false)
                        as ViewGroup
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            if (dockContext.type == TYPE_NAME || dockContext.type == TYPE_ACTION) {
                holder.actionTv?.text = dockContext.name
                if (!dockContext.enable) {
                    holder.actionTv?.setTextColor(
                        context.resources.getColor(R.color.md_theme_dark_outline)
                    )
                } else {
                    holder.actionTv?.setTextColor(
                        context.resources.getColor(R.color.notification_text_color)
                    )
