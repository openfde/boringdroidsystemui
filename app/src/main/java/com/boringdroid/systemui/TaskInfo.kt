...
class TaskInfo(val packageName: String, val program: String) {
    companion object {
        ...
        fun dockTypeToString(dockType: Int): String =
            when (dockType) {
        ...
        fun platformTypeToString(platformType: Int): String =
            when (platformType) {
    ...
    override fun toString(): String {
        ...
        // realActivityComponentName=$realActivityComponentName icon=${if(icon !=
        // null)"Drawable@${Integer.toHexString(icon.hashCode())}" else "null"}
        //            label=$label  state=${stateToString(state)}  action=$action
        // componentName=$componentName  program=$program  dockType=${dockTypeToString(dockType)}
